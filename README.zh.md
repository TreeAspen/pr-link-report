...
可以从 EIN Presswire 之类的分发报告 PDF 中**自动提取媒体 Logo 并与域名建立对应关系**，
之后只要粘贴转载链接，媒体名称、所在地区、Logo 就会自动补全。

<sub>界面支持简体中文与 English，在侧栏右上角切换。</sub>
...
右侧预览里的标题、名称、地区直接点击修改。可编辑的文字：普通点击 = 改字，`Ctrl+点击` = 跳转；
不可编辑的链接（Logo 方块、截图、名录、分享按钮、摘要链接）：普通点击直接在新窗口打开。
...
`logos.js` 与 `sample.js` 随仓库发布，在线版打开即有完整图库和示例；
`logos/` 与 `logos_manifest.json` 只是中间产物，已加入 `.gitignore`（图片都已内嵌进 `logos.js`）。
换成你自己的报告时，重跑那两个脚本覆盖即可。缺少这两个文件时，工具会退回内置的精简示例，其余功能不受影响。

## 环境

浏览器：Chrome / Edge（依赖 IndexedDB、`ClipboardItem`、CSS 多栏）。
